...
void system_init(void)
{
   ...
   // Allow non-cached read/write access to NPU memory regions in case the Edge compiler puts the input/output buffers there
   WRITE_REG(MPU->RNR, MPU_REGION_NUMBER2);
   CLEAR_BIT(MPU->RLAR, MPU_RLAR_EN_Msk);
   WRITE_REG(MPU->RBAR, (0x34200000 | (MPU_ACCESS_OUTER_SHAREABLE << MPU_RBAR_SH_Pos) | (MPU_REGION_ALL_RW << MPU_RBAR_AP_Pos) | (MPU_INSTRUCTION_ACCESS_DISABLE << MPU_RBAR_XN_Pos)));
   WRITE_REG(MPU->RLAR, (0x343c0000 | (MPU_PRIV_INSTRUCTION_ACCESS_DISABLE << MPU_RLAR_PXN_Pos) | (MPU_ATTRIBUTES_NUMBER0 << MPU_RLAR_AttrIndx_Pos) | (MPU_REGION_ENABLE << MPU_RLAR_EN_Pos)));
   ...
   MODIFY_REG(RIFSC->RISC_SECCFGRx[RIF_RISC_PERIPH_INDEX_TIM2 >> RIF_PERIPH_REG_SHIFT], (1UL << (RIF_RISC_PERIPH_INDEX_TIM2 & RIF_PERIPH_BIT_POSITION)), (RIF_ATTRIBUTE_SEC << (RIF_RISC_PERIPH_INDEX_TIM2 & RIF_PERIPH_BIT_POSITION)));
   MODIFY_REG(RIFSC->RISC_PRIVCFGRx[RIF_RISC_PERIPH_INDEX_TIM2 >> RIF_PERIPH_REG_SHIFT], (1UL << (RIF_RISC_PERIPH_INDEX_TIM2 & RIF_PERIPH_BIT_POSITION)), ((RIF_ATTRIBUTE_PRIV >> 1U) << (RIF_RISC_PERIPH_INDEX_TIM2 & RIF_PERIPH_BIT_POSITION)));
...
void system_finalize(void)
{
   ...
   WRITE_REG(RCC->APB1ENSR1, RCC_APB1ENR1_TIM2EN);
   (void)READ_REG(RCC->APB1ENR1);
   MODIFY_REG(RCC->CFGR2, RCC_CFGR2_TIMPRE, RCC_TIMPRES_DIV1 << RCC_CFGR2_TIMPRE_Pos);

   // Configure the TIM2 peripheral to fire once per second
   TIM2->PSC = (SystemCoreClock / 10000) - 1;  // Counter Clock = 10kHz
   TIM2->ARR = 10000 - 1;  // Counter Period = 1s
   MODIFY_REG(TIM2->CR1, (TIM_CR1_DIR | TIM_CR1_CMS | TIM_CR1_CKD |  TIM_CR1_ARPE), TIM_CR1_URS);
   WRITE_REG(TIM2->EGR, TIM_EGR_UG);
   CLEAR_BIT(TIM2->SMCR, (TIM_SMCR_SMS | TIM_SMCR_TS | TIM_SMCR_ETF | TIM_SMCR_ETPS | TIM_SMCR_ECE | TIM_SMCR_ETP));
   CLEAR_BIT(TIM2->CR2, (TIM_CR2_MMS | TIM_SMCR_MSM));

   // Enable TIM2 interrupts and start the timer
   NVIC_SetPriority(TIM2_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 12, 0));
