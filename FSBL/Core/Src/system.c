...
void system_init(void)
{
   ...
   SET_BIT(RCC->APB4ENSR2, RCC_APB4ENR2_BSECEN);
   (void)READ_BIT(RCC->APB4ENR2, RCC_APB4ENR2_BSECEN);
   SET_BIT(RCC->APB4ENSR2, RCC_APB4ENR2_SYSCFGEN);
   ...
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_PWREN);
   ...
   // Enable PLL1 to output at 2400MHz  // TODO: LOWER IS BETTER FOR POWER CONSUMPTION
   ...
   MODIFY_REG(RCC->PLL1CFGR1, (RCC_PLL1CFGR1_PLL1SEL | RCC_PLL1CFGR1_PLL1DIVM | RCC_PLL1CFGR1_PLL1DIVN), (RCC_PLLSOURCE_HSI | (2U << RCC_PLL1CFGR1_PLL1DIVM_Pos) | (75U << RCC_PLL1CFGR1_PLL1DIVN_Pos)));
   ...
   WRITE_REG(RCC->IC1CFGR, (RCC_ICCLKSOURCE_PLL1 | ((4U - 1U) << RCC_IC1CFGR_IC1INT_Pos)));   // IC1 used for CPU clock @ 600MHz
   ...
   WRITE_REG(RCC->IC2CFGR, RCC_ICCLKSOURCE_PLL1 | ((6U - 1U) << RCC_IC2CFGR_IC2INT_Pos));     // IC2 used for SYS clock @ 400MHz
   WRITE_REG(RCC->IC6CFGR, RCC_ICCLKSOURCE_PLL1 | ((3U - 1U) << RCC_IC6CFGR_IC6INT_Pos));     // IC6 used for NPU @ 800MHz
   WRITE_REG(RCC->IC11CFGR, RCC_ICCLKSOURCE_PLL1 | ((3U - 1U) << RCC_IC11CFGR_IC11INT_Pos));  // IC11 used for AXISRAM 3-6 @ 800MHz
   ...
   SET_BIT(RCC->AHB3ENSR, RCC_AHB3ENR_RIFSCEN);
   ...
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_GPIODEN);
   ...
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_GPIOFEN);
   (void)READ_BIT(RCC->AHB4ENR, RCC_AHB4ENR_GPIOFEN);
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_GPIOGEN);
   ...
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_GPIOOEN);
   (void)READ_BIT(RCC->AHB4ENR, RCC_AHB4ENR_GPIOOEN);
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_GPIOPEN);
   ...
   // Power on and enable all AXISRAM memory banks and the ICACHE  // TODO: CAN ANY AXISRAM BANKS NOT BE ENABLED (NOT USED IN AI)
   SET_BIT(RCC->AHB4ENSR, RCC_AHB4ENR_CRCEN);  // TODO: DO I NEED TO ENABLE THE CRC CLOCK
   (void)READ_BIT(RCC->AHB4ENR, RCC_AHB4ENR_CRCEN);
   SET_BIT(RCC->AHB2ENSR, RCC_AHB2ENR_RAMCFGEN);
   (void)READ_BIT(RCC->AHB2ENR, RCC_AHB2ENR_RAMCFGEN);
   SET_BIT(RCC->MEMENSR, RCC_MEMENR_CACHEAXIRAMEN);
   (void)READ_BIT(RCC->MEMENR, RCC_MEMENR_CACHEAXIRAMEN);
   SET_BIT(RCC->MEMENSR, RCC_MEMENR_AXISRAM2EN);
   (void)READ_BIT(RCC->MEMENR, RCC_MEMENR_AXISRAM2EN);
   SET_BIT(RCC->MEMENSR, RCC_MEMENR_AXISRAM3EN);
   (void)READ_BIT(RCC->MEMENR, RCC_MEMENR_AXISRAM3EN);
   SET_BIT(RCC->MEMENSR, RCC_MEMENR_AXISRAM4EN);
   (void)READ_BIT(RCC->MEMENR, RCC_MEMENR_AXISRAM4EN);
   SET_BIT(RCC->MEMENSR, RCC_MEMENR_AXISRAM5EN);
   (void)READ_BIT(RCC->MEMENR, RCC_MEMENR_AXISRAM5EN);
   SET_BIT(RCC->MEMENSR, RCC_MEMENR_AXISRAM6EN);
   (void)READ_BIT(RCC->MEMENR, RCC_MEMENR_AXISRAM6EN);
   ...
   // Enable the NPU clock and reset the NPU peripheral  // TODO: DISABLE WHEN NOT IN USE
   SET_BIT(RCC->AHB5ENSR, RCC_AHB5ENR_NPUEN);
   ...
   // Enable the NPU cache clock and reset the NPU cache  // TODO: DISABLE WHEN NOT IN USE
   SET_BIT(RCC->AHB5ENSR, RCC_AHB5ENR_CACHEAXIEN);
